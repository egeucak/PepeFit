...
public class ListMemberBean {
	...
	public ArrayList<MemberTemp> getUserNames() {
		ArrayList<MemberTemp> userNames = new ArrayList<MemberTemp>();
		...
		for (LinkedHashMap<String, Object> member : members) {
			// System.out.println("NAME : " + member.get("NAME").toString() + " " +
			// member.get("SURNAME").toString());
			// System.out.println("MAIL : " + member.get("EMAIL"));
			userNames.add(new MemberTemp(member.get("NAME").toString() + " " + member.get("SURNAME").toString(),
					member.get("TC").toString()));
		}
	...
	// Course course = new Course((Integer)
	// row.get("C_ID"),row.get("C_NAME").toString(),row.get("C_DESCRIPTION").toString());
	// courses.add(course);
	// x++;
	// }
	// }
